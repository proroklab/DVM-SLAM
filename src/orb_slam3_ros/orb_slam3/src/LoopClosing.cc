...
bool LoopClosing::DetectAndReffineSim3FromLastKF(KeyFrame* pCurrentKF, KeyFrame* pMatchedKF, g2o::Sim3& gScw,
  int& nNumProjMatches, std::vector<MapPoint*>& vpMPs, std::vector<MapPoint*>& vpMatchedMPs) {
...
bool LoopClosing::DetectCommonRegionsFromBoW(std::vector<KeyFrame*>& vpBowCand, KeyFrame*& pMatchedKF2,
  KeyFrame*& pLastCurrentKF, g2o::Sim3& g2oScw, int& nNumCoincidences, std::vector<MapPoint*>& vpMPs,
  ...
  for (KeyFrame* pKFi : vpBowCand) {
    ...
    if (numBoWMatches >= nBoWMatches) // TODO pick a good threshold
    {
      ...
      while (!bConverge && !bNoMore) {
        ...
        // to_string(nInliers)
        // + " geometrical inliers among " + to_string(nBoWInliers) + "
        // BoW matches", Verbose::VERBOSITY_DEBUG);
      }
      ...
      if (bConverge) {
        ...
        g2o::Sim3 gSmw(
          pMostBoWMatchesKF->GetRotation().cast<double>(), pMostBoWMatchesKF->GetTranslation().cast<double>(), 1.0);
...
int LoopClosing::FindMatchesByProjection(KeyFrame* pCurrentKF, KeyFrame* pMatchedKFw, g2o::Sim3& g2oScw,
  set<MapPoint*>& spMatchedMPinOrigin, vector<MapPoint*>& vpMapPoints, vector<MapPoint*>& vpMatchedMapPoints) {
  int nNumCovisibles = 10;
...
void LoopClosing::MergeLocal() {
  ...
  if (vpCurrentMapKFs.size() == 0) { }
    ...
    if (mpTracker->mSensor == System::MONOCULAR) {
      unique_lock<mutex> currentLock(pCurrentMap->mMutexMapUpdate); // We update the current map with
...
void LoopClosing::MergeLocal2() {
  ...
  /*cout << "vpCurrentConnectedKFs.size() " << vpCurrentConnectedKFs.size() <<
    endl; cout << "mvpMergeConnectedKFs.size() " <<
    mvpMergeConnectedKFs.size()
    ...
    if (!mbStopGBA) {
      ...
      while (!lpKFtoCheck.empty()) {
        ...
        /*cv::Mat Tco_cn = pKF->mTcwBefGBA * pKF->mTcwGBA.inv();
                ...
                cout << "GBA: KF " << pKF->mnId << " had been moved " <<
           dist << " meters" << endl; double desvX = 0; double desvY =
           ...
           endl; cout
           << "--It has " << pKF->GetCovisiblesByWeight(80).size() << "
           ...
           cv::circle(imLeft, pKF->mvKeys[i].pt, 2, cv::Scalar(0, 255,
           0)); cv::putText(imLeft, strNumOBs, pKF->mvKeys[i].pt,
           CV_FONT_HERSHEY_DUPLEX, 1, cv::Scalar(255, 0, 0));
                    }
                    cout << "--It has " << num_MPs << " MPs matched in
           the map"
           << endl;

                    string namefile = "./test_GBA/GBA_" +
           to_string(nLoopKF) +
