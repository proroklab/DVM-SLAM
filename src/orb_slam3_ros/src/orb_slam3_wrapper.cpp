...
void OrbSlam3Wrapper::receiveNewKeyFrames(const interfaces::msg::NewKeyFrames::SharedPtr msg) {
  ...
  vector<ORB_SLAM3::KeyFrame*> existingKeyFrames = pSLAM->GetAtlas()->GetAllKeyFrames();
  vector<ORB_SLAM3::KeyFrame*> keyFramesInLocalMappingQueue = pSLAM->GetLocalMapper()->GetKeyFramesInQueue();
  existingKeyFrames.insert(
    existingKeyFrames.end(), keyFramesInLocalMappingQueue.begin(), keyFramesInLocalMappingQueue.end());
  cout << "keyFramesInLocalMappingQueue.size(): " << keyFramesInLocalMappingQueue.size() << "\n";

  vector<ORB_SLAM3::MapPoint*> existingMapPoints = pSLAM->GetAtlas()->GetAllMapPoints();

  ORB_SLAM3::Map* newMap
    = pSLAM->GetAtlas()->DeserializeMap(msg->serialized_map, true, existingKeyFrames, existingMapPoints);
